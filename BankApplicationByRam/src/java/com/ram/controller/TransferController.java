...
public class TransferController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String toAccountNumber=request.getParameter("toacno");
            String tdate=request.getParameter("tdate");
            String tamount=request.getParameter("tamt");
            TransferBean tb=new TransferBean();
            tb.setAccountNumber(acno);
            tb.setBeneficiaryAccountnumber(toAccountNumber);
            tb.setTransactionAmount(tamount);
            tb.setTransactionDate(tdate);
            BankDAO bd=new BankDAO();
          boolean b=  bd.transfertMoney(tb);
            if(b){
                out.println("<h1><font color='green'> Amount Transfer Successs</font></h1>");
            }
